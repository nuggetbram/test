...
const string strMessageMagic = "marucoin Signed Message:\n";
...
{
    // marucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // marucoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // marucoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // marucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, marucoin - DarkGravity v2, written by Evan Duffield - evan@marucoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
{
        static const int64 BlocksTargetSpacing = 3 * 60; // 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
            ...
            if (pindexLast->nHeight+1 >= 140) { DiffMode = 3; }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // marucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // marucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000a755a7693497db471ee0e588c26d9719cc762aa8515f7c7a9a20ca57358");
    }
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xde }; // marucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("marucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("marucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static marucoinMiner(CWallet *pwallet)
{
    printf("marucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("marucoin-miner");
    ...
    try { loop {
        ...
        printf("Running marucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("marucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&marucoinMiner, pwallet));
}
